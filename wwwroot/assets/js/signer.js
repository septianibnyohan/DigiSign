 ...
 */
 function launchEditor(){
   ...
   if (typeof requestPositions !== 'undefined') {
     requestPositions.forEach(function (requestPosition) {

       if (requestPosition.type == "image") {
         renderPage(parseInt(requestPosition.page))
       }
   ...
   //if (typeof requestPositions !== 'undefined') {
     //renderPage(parseInt(requestPositions[1].page));
   //}
   //renderPage(pageNum);
   //renderPage(requestPositions[1].page);
   // alert("You must sign document on page "+pageNum);
 };
 ...
 */
//   $('input[name=receivers], input[name=email_recipients]').on('beforeItemAdd', function(event) {
 ...
 */
 function validateRequest(){
   ...
   if ($("input[name=restricted]").prop("checked")) {
     ...
     enableTools("request");
     $(".request-helper").show();
     $(".request-helper strong").text(recipients[0].employee_name);
     $(".right-bar.input-fields-list").toggleClass("open");

     $("#inputField").text(recipients[0].employee_name);
   }else{
     sendRequest();
   }
 ...
 */
 function nextRecipient(){
     ...
     emptyAssembler()
     $(".right-bar.input-fields-list").toggleClass("open");
   $(".request-helper strong").text(recipients[recipientIndex].employee_name);
     $("#inputField").text(recipients[recipientIndex].employee_name);
 }
 ...
 */
 function sendRequest(){
   var name = JSON.stringify(recipients),
       emails = JSON.stringify(email_recipients),
...
$(".signer-save").click(function (event) {
 ...
 if (inviting) {
   ...
   if (faceAuth == "ON") {
     $("#LoadFaceRecognation").modal("show");
     callAjaxCurlFace();
   } else {
     $("#LoadInputPassword").modal('show');
   }
...
function callAjaxCurlFace(){
     $(".loader").show();

    $.ajax({
      "async": true,
      "crossDomain": true,
     "url": baseUrl+"Api/Fetch",
     "method": "GET",
     "headers": {
       "Accept": "*/*",
         ...
         // $('#response').html(response);
           ...
           if(data.status == true){
             window.setTimeout(function(){
               // $("#LoadFaceRecognation").modal('hide');

               // $(".loader").hide();
               $.ajax({
                 "async": true,
                 "crossDomain": true,
                "url": baseUrl+"api/fetch/callbackWebhook?refId="+data.messages,
                "method": "GET",
                "headers": {
                  "Accept": "*/*",
                  "Cache-Control": "no-cache",
                ...
                success: function (response) {
                    ...
                    var res = JSON.parse(response);
                     ...
                     if(res.status){
                       ...
                       $(".loader").hide();
                       toastr.error("Failed Retrieved Data", "Face Auth Not Recognized As Approved ");
                       $("#LoadFaceRecognation").modal("hide");
                       $("#LoadInputPassword").modal('show');
                       // alert(res.messages);
                     }
             ...
             toastr.warning("Failed Retrieved Data", "Face Auth Not Recognized As Approved, Please login with LDAP ");
           $(".loader").hide();
           $("#LoadFaceRecognation").modal("hide");
            $("#LoadInputPassword").modal('show');
         }
     ...
     //  $("#LoadInputPassword").modal('show');
         ...
         //     "hideMethod": "fadeOut"
         //   }
         toastr.warning(msg, "Face Auth Not Recognized As Approved, Please login with LDAP ",5000);
       $(".loader").hide();
       $("#LoadFaceRecognation").modal("hide");
        $("#LoadInputPassword").modal('show');
         $('#response').html(msg);
...
function callAjaxFaceAuth(){

 var data = new FormData();
 data.append("grant_type","password");
 data.append("scope","read");
 data.append("client_id","integra");
 ...
 xhr.open("POST", "http://10.255.4.36/oauth/token");
 xhr.setRequestHeader("Authorization","Basic aW50ZWdyYTo/LmJiUCpLeCRuOWsiJz5D");
 xhr.setRequestHeader("WWW-AUTHENTICATE","Basic aW50ZWdyYTo/LmJiUCpLeCRuOWsiJz5D");
...
function callAjaxFaceAuth2(){

 var formData = {};
 formData['grant_type'] = 'password';
 formData['scope'] = 'read';
 formData['client_id'] = 'integra';


 $.ajax({
   type : "POST",
   url : FACE_API+"oauth/token",
   data: JSON.stringify(formData),
   headers: {
     "Authorization" : "Basic aW50ZWdyYTo/LmJiUCpLeCRuOWsiJz5D",
...
function prepareData(save){
 ...
 if (save) { saveChanges(actions); } else { /*$('#submitPassword').attr('disabled', false);*/return actions; }
}
...
$(".input-field-list").on("click", ".input-field-item div", function(event){
 ...
 if (fieldLabel == "Signature") {
   $('<div class="signer-element selected-element" status="drop" resizeable="true" type="image" group="input" page="'+pageNum+'"><img src="'+baseUrl+'assets/images/signhere.png" style="width:161px;opacity:0.5;"></div>').appendTo(".signer-builder");
 }else{
  $('<div class="signer-element selected-element" status="drop" resizeable="true" type="image" group="input" page="'+pageNum+'"><img src="'+baseUrl+'assets/images/qrcode3.png" style="width:161px;opacity:0.5;"></div>').appendTo(".signer-builder");
 ...
 if ($('#inputField').text() == recipients[1].employee_name)
 {
    $('#input-field-item').text('PLACE QR');
 }
...
$(".accept-request").click(function (event) {
 ...
 //renderPage(requestPositions[1].page);
 //jumpPage(parseInt(requestPositions[1].page));
})
   ...
   if ($(".signer-builder").is(':empty')) {
     ...
     if ($("body").hasClass("accept") && requestPositions.length) {
       ...
       $.each( requestPositions, function( i, field ) {
         ...
         yPos = parseFloat((parseFloat(signerScaler(field.yPos)) + currentDocOffset.top) - currentOffset.top + topOffset-90 - 5).toFixed(3);
         console.log(yPos);
         if (field.type == "image") {
           $('<div class="signer-element" type="signature" signed="false" group="field" page="'+field.page+'" style="display:none;left:'+xPos+'px;top:'+yPos+'px;position:absolute;width:auto;"><span style="display:block;">'+employeeName+'</span><img src="'+baseUrl+'assets/images/signhere.png" style="width:'+signerScaler(field.width)+'px;"><em style="font-size: 8px;index: 10;z-index: 10;display: inline-block;float:right;"></em></div>').appendTo(".signer-builder");
         }else if(field.type == "text"){
